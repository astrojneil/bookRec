...
### Model
The recommender was built loosely from the Machine Learning Coursera Assignment for recommender systems <a href = "https://www.coursera.org/learn/machine-learning?action=enroll" > Book Recommendation System </a> blog.
...
The recommender system makes use of user-based collaborative filtering. For a given user, the 8 most similar users are found through a k-neighbors algorithm. The recommendations are then made from the books read by these 8 similar users by minimizing the cost function for these 8 users and considering 10 freatures. The top 10 books with the highest expected ratings are then returned as recommendations. 
